...
HCO_VERSION="${HCO_VERSION:-2.2.0}"
HCO_CHANNEL="${HCO_CHANNEL:-${HCO_VERSION:0:3}}"
...
__EOF__
#wait for it
retries=0; while [ $retries -lt 200 ] && ! oc wait --for condition=Available hyperconverged ${HCO_NAME} -n ${TARGET_NAMESPACE} --timeout=2m ; do sleep 15; let retries=$retries+1; done
